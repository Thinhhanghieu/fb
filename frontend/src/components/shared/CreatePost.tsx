...
export function CreatePost() {
  ...
  return (
    <>
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className="flex-1 text-left px-5 py-3 rounded-2xl text-sm text-muted-foreground transition-all duration-300 hover:bg-gray-100 hover:opacity-80"
            style={{ background: 'var(--surface-container-low)' }}
          >
            Bạn đang nghĩ gì vậy, {MOCK_USER.name.split(' ')[0]}?
          </button>
        </div>
        <div className="flex items-center gap-2 pt-1 border-t mt-4 border-gray-100">
          {[
            ...
            <button 
              key={item.label}
              onClick={() => setIsModalOpen(true)}
              className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-2xl text-xs font-bold text-muted-foreground hover:bg-gray-100 transition-all duration-200 group"
            >
      ...
      {isModalOpen && (
        <CreatePostEditor onClose={() => setIsModalOpen(false)} />
      )}
    </>
  );
}
