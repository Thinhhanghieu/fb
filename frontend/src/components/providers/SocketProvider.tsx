...
export const SocketProvider: React.FC<{ children: React.ReactNode }> = ({
	...
	const subscriptionQueueRef = useRef<
		{ destination: string; callback: (message: IMessage) => void; id: string }[]
	>([]);
	...
	useEffect(() => {
		...
		const client = new Client({
			...
			onConnect: (frame) => {
				...
				console.log(
					`[WebSocket] Processing queue of ${queue.length} subscriptions`,
				);
				queue.forEach((item) => {
		...
		(destination: string, callback: (message: IMessage) => void) => {
			...
			return () => {
				if (activeSubscriptionsRef.current[id]) {
					...
					subscriptionQueueRef.current = subscriptionQueueRef.current.filter(
						(item) => item.id !== id,
					);
				}
			};
	...
	const publish = React.useCallback((destination: string, body: any) => {
		if (!stompClientRef.current?.connected) {
			console.warn(
				`[WebSocket] Skip Publish - Not connected to ${destination}`,
			);
			return;
		}
